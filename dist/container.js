...
import { LinearIssueService, FSWorkspaceService, NodeClaudeService, ExpressWebhookService } from './adapters/index.js';
...
export function createContainer() {
    ...
    container.register('workspaceService', (c) => {
        ...
        return new FSWorkspaceService(config.workspace.baseDir, fileSystem, processManager, config.workspace.mainBranch);
    });
    ...
    container.register('issueService', (c) => {
        ...
        const service = new LinearIssueService(linearClient, null, // userId will be fetched from API instead of config
        sessionManager, {
        ...
        service.fetchUserData = async () => {
            try {
                ...
                try {
                    ...
                    const agentUser = users.nodes.find((user) => user.name && (user.name.includes('Agent') ||
    ...
    container.register('claudeService', (c) => {
        ...
        return new NodeClaudeService(config.claude.path, config.claude.promptTemplatePath, {
            createComment: (...args) => issueService.createComment(...args)
        }, fileSystem, processManager, attachmentDownloader);
    });
    ...
    container.register('webhookService', (c) => {
        ...
        const oauthHelper = c.get('oauthHelper');
        return new ExpressWebhookService(config.linear.webhookSecret, issueService, httpServer, oauthHelper);
    });
    return container;
}
