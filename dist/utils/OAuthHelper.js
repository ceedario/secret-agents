...
export class OAuthHelper {
    ...
    _saveState(state) {
        ...
        this.fileSystem.writeFileSync(statePath, JSON.stringify(stateData, null, 2));
    }
    /**
    ...
    async _loadState() {
        ...
        try {
            if (await this.fileSystem.pathExists(statePath)) {
                const stateData = JSON.parse(await this.fileSystem.readFile(statePath, 'utf-8'));
    ...
    async _saveTokenInfo(tokenInfo) {
        ...
        await this.fileSystem.writeFile(tokenPath, JSON.stringify(tokenData, null, 2));
    }
    /**
