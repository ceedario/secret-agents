...
export class EdgeWorker extends EventEmitter {
  ...
  private async handleIssueAssigned(issue: any, repository: RepositoryConfig): Promise<void> {
    console.log(
      `[EdgeWorker] handleIssueAssigned started for issue ${issue.identifier} (${issue.id})`
    );
    ...
    try {
      ...
      console.log(
        `[EdgeWorker] Initial prompt built successfully, length: ${prompt.length} characters`
      );
  ...
  private async handleNewComment(
    ...
    console.log(
      `Stored reply context for issue ${issue.id}: commentId=${comment.id}, replyParentId=${replyParentId}`
    );
        ...
        onEvent: (event) => {
          ...
          if (event.type === 'assistant' && 'message' in event && event.message?.content) {
            ...
            for (const item of content) {
              if (
                item?.type === 'text' &&
                item.text?.includes('tool_use` ids were found without `tool_result` blocks')
              ) {
  ...
  private async handleClaudeEvent(
    ...
    if (event.type === 'assistant') {
        ...
        for (const item of messageContent) {
          if (item && typeof item === 'object' && 'type' in item && item.type === 'tool_use') {
            ...
            if (
              ...
              console.log(
                `[EdgeWorker] Detected TodoWrite tool use with ${item.input.todos.length} todos`
              );
      ...
      const errorMessage =
        'message' in event ? event.message : 'error' in event ? event.error : 'Unknown error';
  ...
  private async buildInitialPrompt(
    ...
    try {
      ...
      if (attachmentManifest) {
        ...
        console.log(
          `[EdgeWorker] Final prompt with attachments, total length: ${finalPrompt.length} characters`
        );
        return finalPrompt;
      }

      console.log(
        `[EdgeWorker] Returning prompt without attachments, length: ${prompt.length} characters`
      );
      return prompt;
    } catch (error) {
...
Please analyze this issue and help implement a solution.`;
  ...
  private async reportStatus(update: StatusUpdate): Promise<void> {
    ...
    const promises = Array.from(this.ndjsonClients.values()).map((client) =>
      client.sendStatus(update).catch((err) => console.error('Failed to send status update:', err))
    );
   ...
   */
  private async postInitialComment(
    issueId: string,
    repositoryId: string
  ): Promise<{ id: string } | null> {
    try {
      const body =
        "I've been assigned to this issue and am getting started right away. I'll update this comment with my plan shortly.";
   ...
   */
  private async updateCommentWithTodos(
    issueId: string,
  ...
  private generateAttachmentManifest(downloadResult: {
    ...
    const { attachmentMap, imageMap, totalFound, downloaded, imagesDownloaded, skipped, failed } =
      downloadResult;
