...
export class HttpServer {
  ...
  jsonParser(): RequestHandler[] {
    ...
    const jsonErrorHandler: RequestHandler = (req: Request, _res: Response, next: NextFunction): void => {
      // This is actually an error handler, but for type compatibility we'll make it a regular middleware
      if ((req as any).jsonError) {
        console.error('JSON parsing error:', (req as any).jsonError.message);
