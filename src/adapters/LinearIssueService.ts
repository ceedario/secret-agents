...
import { LinearIssueSchema, LinearIssueType } from '../utils/schemas.js';
...
export class LinearIssueService extends IssueService {
  ...
  _convertToDomainIssue(linearIssue: LinearIssueType): Issue {
    ...
    let comments: any[] = [];
    if (Array.isArray((linearIssue as any).comments)) {
      comments = (linearIssue as any).comments.map((comment: any) => ({
        id: comment.id,
        body: comment.body,
    ...
    return new Issue({
      ...
      state: (linearIssue as any).state?.name || (linearIssue as any).state || null,
      stateType: (linearIssue as any).state?.type || null,
      priority: linearIssue.priority,
      url: linearIssue.url,
      assigneeId: linearIssue._assignee?.id || (linearIssue as any).assignee?.id || (linearIssue as any).assigneeId,
   ...
   * @inheritdoc
   */
  override async fetchAssignedIssues(): Promise<Issue[]> {
    try {
      // Ensure we have user ID from API before proceeding
      if (!this.userId && (this as any).fetchUserData) {
        console.log('No user ID available, fetching from API first...');
        const success = await (this as any).fetchUserData();
        if (!success || !this.userId) {
          ...
          if (allIssues && allIssues.nodes && allIssues.nodes.length > 0) {
            ...
            return allIssues.nodes.map(issue => this._convertToDomainIssue(issue as unknown as LinearIssueType))
      ...
      return issues.nodes.map(issue => this._convertToDomainIssue(issue as unknown as LinearIssueType));
    } catch (error: any) {
   ...
   */
  override getAuthStatus(): boolean {
   ...
   * @inheritdoc
   */
  override async fetchIssue(issueId: string): Promise<Issue> {
    try {
      // Fetch the issue details
      const issueResponse = await this.linearClient.issue(issueId);
      
      // Fetch comments for the issue
      const commentsResponse = await (this.linearClient as any).issueComments(issueId);
      ...
      return this._convertToDomainIssue(issueWithComments as unknown as LinearIssueType);
    } catch (error) {
   ...
   * @inheritdoc
   */
  override async createComment(issueId: string, body: string): Promise<boolean> {
   ...
   */
  async initializeIssueSession(issue: Issue, isStartupInit: boolean = false): Promise<void> {
   ...
   * @inheritdoc
   */
  override async handleIssueCreateEvent(issueData: IssueData): Promise<void> {
    try {
   ...
   * @inheritdoc
   */
  override async handleIssueUpdateEvent(issueData: IssueData): Promise<void> {
    try {
   ...
   * @inheritdoc
   */
  override async handleCommentEvent(commentData: IssueData): Promise<void> {
    try {
   ...
   * @inheritdoc
   */
  override async handleAgentMention(data: AgentHandlerData): Promise<void> {
    try {
   ...
   * @inheritdoc
   */
  override async handleAgentAssignment(data: AgentHandlerData): Promise<void> {
    try {
      // Extract relevant information from notification data
      const { issueId } = data;
      const actor = data.actor?.name || 'Unknown user';
   ...
   * @inheritdoc
   */
  override async handleAgentReply(data: AgentHandlerData): Promise<void> {
    try {
   ...
   * @inheritdoc
   */
  override async handleAgentUnassignment(data: AgentHandlerData): Promise<void> {
    try {
