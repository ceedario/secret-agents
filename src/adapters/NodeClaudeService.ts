...
export class NodeClaudeService extends ClaudeService {
   ...
   * @private
   */
  async _initPromptTemplate(): Promise<void> {
    try {
   ...
   */
  async _loadPromptTemplate(templatePath: string): Promise<string> {
    try {
      if (!templatePath) {
   ...
   * @inheritdoc
   */
  override async buildInitialPrompt(issue: Issue, tokenLimitResumeContext: boolean = false, attachmentManifest: string = ''): Promise<string> {
...
`;
   ...
   */
  _setupClaudeProcessHandlers(
    claudeProcess: ChildProcess, 
    issue: Issue, 
    workspace: Workspace, 
    historyPath: string, 
    onTokenLimitError: TokenLimitCallback | null = null
  ): ChildProcess {
    ...
    claudeProcess.stdout?.on('data', async (data: Buffer) => {
      ...
      for (let i = 0; i < lines.length - 1; i++) {
        ...
        try {
          const jsonResponse: ClaudeJsonResponse = JSON.parse(line);
          ...
          if (jsonResponse.type === 'assistant' && jsonResponse.message) {
            ...
            if (currentResponseText.trim().length > 0) {
              ...
              if (currentResponseText !== 'Prompt is too long') {
                ...
                if (!firstResponsePosted) {
                  ...
                  (issue as any).firstResponseContent = lastAssistantResponseText.trim();
            ...
            if (message.stop_reason === 'end_turn') {
              ...
              if (lastAssistantResponseText.trim().length > 0 && lastAssistantResponseText !== 'Prompt is too long') {
                ...
                if (!firstResponsePosted || this._isContentChanged((issue as any).firstResponseContent, lastAssistantResponseText)) {
                  console.log(
          ...
          const isTokenLimitError = (
            ...
            (jsonResponse.type === 'error' && 
             ...
             ((jsonResponse as any).message === 'Prompt is too long' || 
              (jsonResponse as any).message?.toLowerCase().includes('prompt is too long'))) ||
            // Error object
            (jsonResponse.error && 
      ...
      if (line) {
        try {
          ...
          for (const part of parts) {
            ...
            try {
              const jsonResponse: ClaudeJsonResponse = JSON.parse(part);
   ...
   */
  async startFreshSessionAfterTokenLimit(issue: Issue, workspace: Workspace): Promise<Session> {
   ...
   * @inheritdoc
   */
  override async startSession(issue: Issue, workspace: Workspace): Promise<Session> {
    return new Promise(async (resolve, reject) => {
      try {
...
CLAUDE_INPUT_EOF`;
        ...
        this._setupClaudeProcessHandlers(claudeProcess, issue, workspace, historyPath || '', async (issue: Issue, workspace: Workspace) => {
          ...
          try {
            ...
            if (this.issueService && (this.issueService as any).sessionManager) {
              (this.issueService as any).sessionManager.updateSession(issue.id, freshSession);
            }
   ...
   * @inheritdoc
   */
  override async sendComment(session: Session, commentText: string): Promise<Session> {
    return new Promise(async (resolve, reject) => {
      try {
...
CLAUDE_INPUT_EOF`;
        ...
        newClaudeProcess.on('error', (err: Error) => {
        ...
        this._setupClaudeProcessHandlers(newClaudeProcess, issue, workspace, historyPath || '', async (issue: Issue, workspace: Workspace) => {
