...
export class LinearIssueService extends IssueService {
  ...
  _convertToDomainIssue(linearIssue) {
    ...
    return new Issue({
      ...
      url: linearIssue.url,
      assigneeId:
        linearIssue.assignee?.id ||
        linearIssue._assignee?.id ||
        linearIssue.assigneeId,
      comments: comments,
    ...
    try {
      ...
      if (!this.userId && this.fetchUserData) {
        ...
        if (!success || !this.userId) {
          console.error(
            "Could not automatically determine the agent user ID. Will attempt to fetch all open issues instead."
          );
          ...
          if (allIssues && allIssues.nodes && allIssues.nodes.length > 0) {
            ...
            return allIssues.nodes
              .map((issue) => this._convertToDomainIssue(issue))
      ...
      if (!this.userId) {
        console.error(
          "No user ID available - will return an empty list of assigned issues"
        );
        return [];
      }
      ...
      if (
        ...
        throw new Error(
          `Authentication required. Please use the OAuth flow to authenticate with Linear.`
        );
      } else {
        // For other errors, provide a concise message
        console.error(
          `Error fetching assigned issues: ${error.message || String(error)}`
        );
        throw error;
      }
    }
  ...
  async fetchIssue(issueId) {
    try {
      ...
      if (process.env.DEBUG_LINEAR_API === "true") {
        ...
        console.log(
          `Fetched issue: ${issueResponse.identifier} - ${issueResponse.title}`
        );
        console.log(
          `Fetched ${commentCount} comments for issue ${issueResponse.identifier}`
        );
      }
  ...
  async findRootCommentId(issueId, commentId) {
    try {
      ...
      while (
        currentComment.parent?.id &&
        !visited.has(currentComment.parent.id)
      ) {
        visited.add(currentComment.parent.id);
        currentComment = comments.find(
          (c) => c.id === currentComment.parent.id
        );
      ...
      console.error(
        `Error finding root comment for ${commentId} in issue ${issueId}:`,
        error
      );
      return null;
    }
  ...
  async createCommentAndGetId(issueId, body, parentId = null) {
    ...
    console.log(
      `Comment length: ${bodyLength} characters, Preview: ${bodyPreview}`
    );
    if (parentId) {
    ...
    try {
      ...
      const commentId =
        ...
        response?.comment?.id ||
        response?.commentCreate?.comment?.id;
  ...
  async updateComment(commentId, body) {
    ...
    try {
      ...
      const response = await this.linearClient.updateComment(commentId, {
        body,
      });
  ...
  async initializeIssueSession(
    ...
    try {
      ...
      if (!workspace) {
        ...
        console.log(
          `Starting Claude session for issue ${issue.identifier} with agent user ID: ${this.userId}`
        );
        const session = await this.claudeService.startSession(
          issue,
          workspace,
          agentRootCommentId
        );
        ...
        if (isStartupInit) {
          ...
          console.log(
            `Workspace already exists for issue ${issue.identifier}. Skipping automatic Claude session start.`
          );
        } else {
          // For comment/mention triggers, always start a session
          console.log(
            `Starting Claude session for issue ${issue.identifier} with existing workspace`
          );
          const session = await this.claudeService.startSession(
            issue,
            workspace,
      ...
      console.error(error);
      console.error(
        `Error setting up session for issue ${issue.identifier}:`,
        error.message
      );
      ...
      try {
        ...
        console.error(
          `Failed to post error comment to Linear: ${commentError.message}`
        );
      }
    }
  }
  ...
  async handleIssueCreateEvent(issueData) {
    try {
      ...
      console.log(
        `Issue assignee ID: ${issueData.assigneeId || "null (unassigned)"}`
      );
  ...
  async handleIssueUpdateEvent(issueData) {
    try {
      ...
      if (issueData.state && issueData.state.type === "canceled") {
        console.log(
          `Issue ${issueData.identifier || issueData.id} has been canceled`
        );
        ...
        if (this.sessionManager.hasSession(issueData.id)) {
          ...
          console.log(
            `Issue ${session.issue.identifier} has been canceled, terminating Claude process`
          );
          ...
          try {
            ...
            console.error(
              `Failed to post cancellation comment: ${commentError.message}`
            );
          }
          ...
          if (session.process && !session.process.killed) {
            try {
              ...
              console.log(
                `✅ Terminated Claude process for canceled issue ${session.issue.identifier}`
              );
            } catch (killError) {
              console.error(
          ...
          console.log(
            `Session removed for canceled issue ${session.issue.identifier}`
          );
        }
      }
      ...
      if ("assigneeId" in issueData) {
        ...
        console.log(
          `Detected assignee change. New assignee ID: ${
            newAssigneeId || "null (unassigned)"
          }`
        );
        console.log(`Our agent user ID: ${this.userId}`);
        ...
        if (this.sessionManager.hasSession(issueData.id)) {
          ...
          console.log(
            `Previous assignee ID: ${previousAssigneeId || "null (unassigned)"}`
          );
    ...
    try {
      ...
      if (user.id === this.userId) {
        ...
        if (process.env.DEBUG_SELF_WEBHOOKS === "true") {
          console.log(
            `⚠️ Skipping comment from our own user (${user.id}) to prevent infinite loop`
          );
        }
        return;
      }
      ...
      console.log(
        `Checking for agent mention. Agent username: ${this.username}`
      );
      ...
      if (session) {
        ...
        try {
          ...
          if (commentData.parentId) {
            ...
            const rootCommentId = await this.findRootCommentId(
              issueId,
              commentData.id
            );
          ...
          const updatedSession = await this.claudeService.sendComment(
            session,
            body
          );
  ...
  async handleAgentMention(data) {
    try {
      ...
      console.log(
        `Agent mentioned by ${actor} in comment ${commentId} on issue ${issueId}`
      );
      if (commentContent) {
        console.log(
          `Comment content (preview): ${commentContent.substring(0, 100)}${
      ...
      if (this.sessionManager.hasSession(issueId)) {
        ...
        if (commentContent) {
          console.log(
            `Processing mention as a comment: ${commentContent.substring(
              0,
              50
        ...
        console.log(
          `No existing session for issue ${issue.identifier}, initializing new session`
        );
  ...
  async handleAgentAssignment(data) {
    try {
      ...
      try {
        ...
        console.error(
          `Failed to move issue to "started" state: ${stateError.message}`
        );
      ...
      if (firstCommentResult?.success && firstCommentResult?.commentId) {
        ...
        await this.initializeIssueSession(
          issue,
          false,
          firstCommentResult.commentId
        );
      } else {
        console.log(
          `Starting work on issue ${issue.identifier} (no comment ID available)`
        );
  ...
  async handleAgentReply(data) {
    try {
      ...
      console.log(
        `Reply from ${actor} on issue ${data.issue?.identifier || issueId}`
      );
      if (commentContent) {
        console.log(
          `Comment preview: ${commentContent.substring(0, 50)}${
            commentContent.length > 50 ? "..." : ""
          }`
        );
      }
  ...
  async handleAgentUnassignment(data) {
    try {
      ...
      console.log(
        `Agent unassigned from issue ${issue.identifier} by ${actor.name}`
      );
      ...
      if (this.sessionManager.hasSession(issueId)) {
        ...
        if (session.process && !session.process.killed) {
          console.log(
            `Terminating Claude process for issue ${issue.identifier}`
          );
        ...
        try {
          ...
          console.error(
            `Failed to post unassignment comment to Linear: ${commentError.message}`
          );
        }
      } else {
  ...
  async moveIssueToInProgress(issueId) {
    try {
      ...
      if (process.env.DEBUG_LINEAR_API === "true") {
        console.log(
          `Successfully updated issue ${issueId} to "${startedState.name}" state (type: started)`
        );
      }
    } catch (error) {
